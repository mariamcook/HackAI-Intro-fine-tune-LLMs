...
1. Fine-tune the BERT LLM on debate text labelled as being from UK Labour and Conservative Party politicians' contributions. (For completeness the notebook used to scrape parliamentary debates is also included in this repo). Debates scraped from: https://www.theyworkforyou.com/

2. Locally prompt the InstructABSA model for aspect term extraction and aspect-based sentiment analysis (already fine-tuned version of TK-instruct, an instruction-tuned version of the T5 LLM), using the InstructABSA prompt templates. See here for more information about InstructABSA: https://github.com/kevinscaria/instructabsa
